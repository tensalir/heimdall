  ...
  function runIterate() {
    ...
    figma.ui.onmessage = async (msg) => {
      ...
      if (msg.type === "smart-reframe") {
        try {
          let findAllImageRects2 = function(node) {
            const rects = [];
            ...
            if ("children" in node) {
              for (const c of node.children) rects.push(...findAllImageRects2(c));
            }
            return rects;
          };
          var findAllImageRects = findAllImageRects2;
          ...
          const imageRects = findAllImageRects2(variantFrame);
