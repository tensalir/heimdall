...
function SourcePanel({
  ...
  onSelect: (source: SourceItem | null) => void
}) {
  ...
  return (
    <div className="flex flex-col h-full">
      <div className="px-4 py-3 border-b border-border flex items-center justify-between">
        ...
        >
          {generating ? <Loader2 className="h-3 w-3 animate-spin" /> : <PaintbrushIcon className="h-3 w-3" />}
          Generate
        </Button>
      </div>
      <div className="flex-1 overflow-y-auto p-4">
            ...
            {assets.map((asset) => (
              <div key={asset.id} className="rounded-lg border border-border bg-card overflow-hidden">
                ...
                <div className="px-2 py-1.5">
                  <p className="text-[10px] text-muted-foreground truncate" title={asset.prompt}>{asset.prompt}</p>
...
export function CreateAdsClient({
  ...
  const handleGenerateAssets = useCallback(async () => {
    ...
    setAssets((prev) => [
      ...prev,
      { id: tempId, prompt: sections.visual || sections.idea || 'Generate ad creative', image_url: null, status: 'generating', model: 'nano-banana-2', created_at: new Date().toISOString() },
    ])
    try {
      const res = await fetch('/api/briefing-assistant/generate-asset', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ source_item_id: source.id, briefing_sections: sections, model: 'gemini-nano-banana-2' }),
      })
      const data = await res.json()
      setAssets((prev) =>
        prev.map((a) =>
          a.id === tempId
            ? { ...a, id: data.asset?.id ?? tempId, image_url: data.asset?.image_url ?? null, prompt: data.asset?.prompt ?? a.prompt, status: data.asset ? 'completed' : 'failed' }
            : a,
        ),
      )
    } catch {
